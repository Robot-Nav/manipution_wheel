...
TABLE_HEIGHT = 0.40  # Table surface height in world frame
...
class MujocoSim:
    def __init__(self, mjcf_path, command_queue, shm_state, show_viewer=True):
        ...
        for object_name in ['cube', 'sponge']:
        ...
        # Cache object qpos references (if objects exist in scene)
        self.object_qpos = {}
        for obj_name in ['cube', 'sponge']:
            if obj_name in body_names:
                obj_body = self.model.body(obj_name)
                obj_jnt = self.model.joint(obj_body.jntadr[0])
                obj_qpos_adr = obj_jnt.qposadr[0]
                # freejoint has 7 qpos (3 pos + 4 quat)
                obj_qpos_size = 7 if obj_jnt.type[0] == 0 else obj_body.jntnum.item()
                self.object_qpos[obj_name] = self.data.qpos[obj_qpos_adr:obj_qpos_adr + obj_qpos_size]

        # Sponge grasping state
        self.sponge_grasped = False
        self.sponge_body_id = self.model.body('sponge').id if 'sponge' in body_names else None
    ...
    def reset(self):
        ...
        # Reset sponge grasping state
        self.sponge_grasped = False

        # Reset objects (if they exist in scene)
        if 'cube' in self.object_qpos:
            qpos_cube = self.object_qpos['cube']
            qpos_cube[:2] += np.random.uniform(-0.1, 0.1, 2)
            theta = np.random.uniform(-math.pi, math.pi)
            qpos_cube[3:7] = np.array([math.cos(theta / 2), 0, 0, math.sin(theta / 2)])
    ...
    def control_callback(self, *_):
        ...
        # Sponge grasping: kinematically attach sponge to gripper when grasped
        if self.sponge_body_id is not None and 'sponge' in self.object_qpos:
            sponge_qpos = self.object_qpos['sponge']
            gripper_openness = self.qpos_gripper[0] / 0.8  # 0=closed, 1=open
            sponge_world_pos = self.data.xpos[self.sponge_body_id].copy()
            pinch_world_pos = self.site_xpos.copy()
            dist = np.linalg.norm(sponge_world_pos - pinch_world_pos)

            # Grasp: gripper closed + sponge near pinch point
            if not self.sponge_grasped and gripper_openness < 0.3 and dist < 0.10:
                self.sponge_grasped = True

            # Release: gripper opened
            if self.sponge_grasped and gripper_openness > 0.5:
                self.sponge_grasped = False
                # Place sponge on table surface below current position
                sponge_qpos[2] = TABLE_HEIGHT + 0.016  # Just above table
                sponge_qpos[3:7] = [1, 0, 0, 0]  # Reset orientation
                # Give small downward velocity so it settles naturally
                sponge_jnt = self.model.joint(self.model.body('sponge').jntadr[0])
                sponge_dof_adr = sponge_jnt.dofadr[0]
                self.data.qvel[sponge_dof_adr:sponge_dof_adr + 6] = 0
                self.data.qvel[sponge_dof_adr + 2] = -0.1  # Small downward velocity

            # If grasped, move sponge with gripper
            if self.sponge_grasped:
                # Set sponge position to pinch site + offset below
                sponge_qpos[0:3] = pinch_world_pos + np.array([0, 0, -0.025])
                # Set sponge orientation to gripper orientation
                gripper_body_id = self.model.body('base').id  # 2f85 base body
                sponge_qpos[3:7] = self.data.xquat[gripper_body_id].copy()
                # Zero out sponge velocity
                sponge_jnt = self.model.joint(self.model.body('sponge').jntadr[0])
                sponge_dof_adr = sponge_jnt.dofadr[0]
                self.data.qvel[sponge_dof_adr:sponge_dof_adr + 6] = 0
...
class MujocoEnv:
    def __init__(self, render_images=True, show_viewer=True, show_images=False, mjcf_path=None):
        self.mjcf_path = mjcf_path or 'models/stanford_tidybot/scene.xml'

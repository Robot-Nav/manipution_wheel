...
1. **全向移动底座（Holonomic Base）**：采用动力脚轮（Powered Caster）设计，实现全向运动，可独立且同时控制全部平面自由度 (x, y, theta)，消除非全向底座的运动学约束
...
设底座在全局坐标系下的位姿为 x = [x, y, theta]^T，关节空间速度为 q_dot = [q1_s_dot, q1_d_dot, q2_s_dot, q2_d_dot, q3_s_dot, q3_d_dot, q4_s_dot, q4_d_dot]^T（s 为转向，d 为驱动），则操作空间速度与关节速度的关系为：

q_dot = C(q) · x_dot_local

其中 C 矩阵的各元素由脚轮几何参数决定。

对于第 i 个脚轮，设其转向角为 qi_s，车辆中心到转向轴距离为 (hx_i, hy_i)，脚轮偏移为 bx, by，车轮半径为 r：

**转向行：**

C(2i-1,1) = sin(qi_s) / bx

C(2i-1,2) = -cos(qi_s) / bx

C(2i-1,3) = (-hx_i * cos(qi_s) - hy_i * sin(qi_s)) / bx - 1

**驱动行：**

C(2i,1) = cos(qi_s) / r - by * sin(qi_s) / (bx * r)

C(2i,2) = sin(qi_s) / r + by * cos(qi_s) / (bx * r)

C(2i,3) = (hx_i * sin(qi_s) - hy_i * cos(qi_s)) / r + by * (hx_i * cos(qi_s) + hy_i * sin(qi_s)) / (bx * r)

#### 3.1.2 里程计

操作空间速度通过伪逆从关节速度恢复：

x_dot_local = Cp_dagger · q_dot

其中 Cp_dagger = (Cp^T * Cp)^(-1) * Cp^T * Cq^(-1)

全局坐标系下的速度通过旋转矩阵转换：

x_dot_global = R(theta_bar) · x_dot_local

R(theta_bar) = [[cos(theta_bar), -sin(theta_bar), 0],
                [sin(theta_bar),  cos(theta_bar), 0],
                [0,               0,              1]]

其中 theta_bar = theta + 0.5 * theta_dot_local * dt 为中点积分角度。实测里程计精度：平移漂移 < 1cm/m，旋转漂移 < 1°/360°。
...
给定目标末端位姿 (p_target, q_target) 和当前末端位姿 (p_curr, q_curr)：
...
e_rot = quat2vel(q_err)

总误差 e = [e_pos^T, e_rot^T]^T ∈ R^6

#### 3.2.2 关节更新

计算雅可比矩阵 J ∈ R^(6×7)，关节更新量为：

Δq = J^T * (J * J^T + λ * I)^(-1) * e + (I - J^T * (J * J^T + λ * I)^(-1) * J) · e_null

其中：
- λ = 10^(-12) 为阻尼系数
- e_null = wrap(q_0 - q_curr) 为零空间误差，驱动关节趋向参考构型（Retract）
- 第二项为零空间投影，在不影响任务空间精度的前提下优化关节配置

单次最大角度变化限制为 45°，迭代最多 20 次，收敛阈值 ||e|| < 10^(-4)。
...
τ_task = -K_p * (q_n - q_d) - K_d * (q_dot_n - q_dot_d) + g(q)

其中 q_n, q_dot_n 为名义模型状态，q_d, q_dot_d 为期望轨迹，g(q) 为重力补偿项（由 [Pinocchio](https://github.com/stack-of-tasks/pinocchio) 计算）。

**名义电机模型：**

q_ddot_n = K_r^(-1) * (τ_task - τ_s_f)

q_dot_n ← q_dot_n + q_ddot_n * dt

q_n ← q_n + q_dot_n * dt

**名义摩擦补偿：**

τ_f = K_r * K_l * [(q_dot_n - q_dot_s) + K_lp * (q_n - q_s)]
...
| K_p | 100 | 50 |
| K_d | 3 | 2 |
| K_r | diag(0.3) | diag(0.18) |
| K_l | 75 | 40 |
| K_lp | 5 | 4 |

力矩传感器信号通过低通滤波（α = 0.01）平滑处理。
...
给定观测序列 o_(t-no+1:t)，策略预测动作序列 a_(t:t+na-1)。

**前向过程**（加噪）：对动作添加高斯噪声

a^k = sqrt(α_bar_k) * a^0 + sqrt(1 - α_bar_k) * ε,  ε ~ N(0, I)

**反向过程**（去噪）：训练 U-Net ε_θ 预测噪声

L = E_{k, a^0, ε} [ ||ε - ε_θ(a^k, k, o)||^2 ]

**推理**：从随机噪声开始，经 K 步去噪生成动作

a^(k-1) = (1 / sqrt(α_k)) * (a^k - (1 - α_k) / sqrt(1 - α_bar_k) * ε_θ(a^k, k, o)) + σ_k * z
...
LATENCY_STEPS = ceil(LATENCY_BUDGET / POLICY_CONTROL_PERIOD) = ceil(0.2 / 0.1) = 2
...
p_robot = [-p_z_xr, -p_x_xr, p_y_xr]

q_robot = [-q_z_xr, -q_x_xr, q_y_xr, q_w_xr]

并施加设备相机偏移补偿，使旋转围绕设备中心而非相机：

p_robot ← p_robot + R_robot · δ_camera

其中 δ_camera = [0, 0.02, -0.04] 为 iPhone 14 Pro 的相机偏移。
...
a_action = [p_base, p_arm, quat2rotvec(q_arm), g_gripper] ∈ R^13
...
- 位置控制器增益：kp = 10^6, kv = 5×10^4（x/y），kp = 5×10^4, kv = 10^3（θ）

**Kinova Gen3 机械臂：**
- 7自由度，自重 8.2 kg，负载 4 kg，工作半径 902 mm
- 关节限位：joint_2 ±2.24 rad，joint_4 ±2.57 rad，joint_6 ±2.09 rad
- 大关节执行器：kp = 2000, kv = 100，力限制 ±105 N·m
- 小关节执行器：kp = 500, kv = 50，力限制 ±52 N·m
...
**仿真目标物：**
- 立方体：0.04 × 0.04 × 0.04 m，质量 0.1 kg
- 初始位置随机化：x, y ∈ [-0.1, 0.1]，θ ∈ [-π, π]
